...
일반적인 IT용어 조금 다를 수 있는 디자인패턴의 용어를 명시적으로 설명한다.

서버: 서버는 참조호출(call by reference)를 받는 측 코드를 의미한다. 그리고 디자인 패턴은 서버측에서 적용되고 서비스를 클라이언트 측에 제공한다.

클라이언트: 클라이언트는 참조호출(call by reference)을 하는 측 코드를 의미한다.

행동: 객체의 행동을 의미하며 자바에서는 메소드가 해당된다.

인터페이스: 클라이언트 측에서 호출 할때 사용되는 추상화된 매개체를 의미하며 자바에서는 추상클래스와 인터페이스가 있다.
